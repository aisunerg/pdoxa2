...
const form = useForm({
    ...
    form.name = props.section.name.toString()
    form.materia = props.section.subject.id.toString()
    form.quota = props.section.quota.toString()
    form.shift_id = props.section.shift_id.toString()
    form.profesor = props.section.teacher[0].id.toString() 

const store = () => {
    form.patch(route('section.update', props.section),{
...
<template>
    <form @submit.prevent="store" class=" grid grid-cols-2 gap-4">
        ...
        <div class="flex mt-2 col-span-2 justify-end">
